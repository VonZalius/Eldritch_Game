...
void Attaques::dessiner_zone(Jeu *jeu)
{
    ...
    for (int y = 0; y < jeu->map.T_HAUTEUR; ++y)
    {
        ...
        {
            // Sol partout !
            if (grille_degat[x][y] == Ombré && jeu->map.grille[x][y] == Sol)
            {
            ...
            else if (grille_degat[x][y] == Explosé && jeu->map.grille[x][y] == Sol)
            {
    ...
    if (jeu->attaques.attaqueTimer.getElapsedTime().asSeconds() < jeu->attaques.Time_to[0])
    {
        if (jeu->attaques.paterne_phases == 0)
        {
            for (int y = 0; y < jeu->map.T_HAUTEUR; ++y)
            {
                for (int x = 0; x < jeu->map.T_LARGEUR; ++x)
                { 
                    if (x < jeu->map.T_LARGEUR / 2 && jeu->attaques.grille_degat[x][y] == Normal)
    ...
    if (jeu->attaques.attaqueTimer.getElapsedTime().asSeconds() < jeu->attaques.Time_to[0])
    {
        if (jeu->attaques.paterne_phases == 0)
        {
            for (int y = 0; y < jeu->map.T_HAUTEUR; ++y)
            {
                for (int x = 0; x < jeu->map.T_LARGEUR; ++x)
                { 
                    if (y < jeu->map.T_HAUTEUR / 2 && jeu->attaques.grille_degat[x][y] == Normal)
                        jeu->attaques.grille_degat[x][y] = Ombré;
                }
            }
            jeu->attaques.paterne_phases++;
